...
print(clean_aggregated_df)
# # Calculate pearson's correlation coefficients separately for each column
# coefficients = [np.corrcoef(clean_aggregated_df[col], clean_aggregated_df['Attack Rate'], rowvar=False)[0, 1] for col in columns_to_agg]
# print(coefficients)

### Calculate multi-linear regression coefficients
...
weighted_indicators = [coef * time_aggregated_df[col] for coef, col in zip(coefficients, columns_to_agg)]
