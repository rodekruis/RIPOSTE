...
library(magrittr)
library(plotly) # for the charts
library(shiny) # to run the shiny app
library(bslib) # for themes
library(shinythemes) # for themes
library(shinydashboard)
library(shinyWidgets) # slider, checkboxes, etc
library(htmlwidgets)

# 1. Load data -----------------------------------------------------------------

#Load the GeoJSON file containing country boundaries
outline <- st_read("D:\\Desktop\\510_Global\\internship\\Data\\geoBoundaries-CMR-ADM1.geojson")

#Load the .csv file containing the attribute information
incidence <- read.csv("D:\\Desktop\\510_Global\\internship\\Data\\Slider_Regional_Incidence.csv")
...
ui <- 
   ...
   #Navbar talks about the various panels that would be visible in the application
    navbarPage(h4("Geovisualisation of Cholera in Cameroon"), collapsible = T, id = "nav",
        
   #---------------- Cholera tab ---------- UI code --------------- 
               
   tabPanel(h5("Cholera Incidence Map"),
            div(class="outer",
                #Linking the CSS file for the styling of the dashbard
                tags$head(includeCSS("D:\\Desktop\\510_Global\\Learning-Shiny app\\styles.css")),
   ...
   tabPanel(h5("Outbreak Plots"), 
            ...
            tabsetPanel(
              ...
              tabPanel("Bar and Line Plots",
                       ...
                       p("The first plot visualises the cumulative no. of cases in a bar chart and cumulative no. of deaths represented by a line. The user can hover on the respective region's bar and line chart to know more about the region."),
                       p("This second plot visualises the no. of cases through the reported timeline in the regions."),
                       p("The user can hover on the respective region's line graph to know more about the histroy of Cholera cases in the region. Click on the region in the bar chart below to know more about that particular region."),
                       
                       #Plot - 1
                       plotlyOutput("bar", width = "900px", height = "300px"),
  ...
  #---------------- Data tab ---------- UI code --------------------------------
   ...
   tabPanel(h5("About"),
              ...
              p("The Source Code for processing the dashboard is available from: ",
                tags$a(href='', '')), #to add github link here
              h3("Data Source"), 
            div(
...
server <- function(input, output, session) {
  ...
  #Report cases by regions, unless a region is chosen
  region_data <- reactive ({
    if(!length(category())) {
      return(count(incidence, ADM1_FR, wt = cases))
    }
    
    #Code filters the incidence data set based on the selected region
    incidence %>%
      filter(ADM1_FR %in% category()) %>%
      count(start_date, wt = cases)
  })
  ...
  #Renders Plotly plot of cholera cases by region
  output$bar <-renderPlotly({
    d <- setNames(region_data(), c("x","y"))
    
    plot_ly(d) %>% 
      add_bars(x =~x, y = ~y, color = ~x, colors = custom_palette) %>%
      add_trace(x= ~x, y = ~deaths, type = 'scatter', mode = 'lines+markers', name = 'Deaths') %>%
      layout(xaxis = list(title = category() %||% "Regions in Cameroon"), 
  ...
  data_time <- reactive({
    if(!length(category())) {
      return(count(incidence, ADM1_FR, start_date, wt = cases))
    }
    incidence %>%
      filter(ADM1_FR %in% category()) %>%
      count(cases, start_date, wt = cases)
  })
  ...
  output$time <- renderPlotly ({
    ...
    plot_ly(d) %>%
      add_lines(x = ~x, y = ~y, color= ~color, colors = custom_palette
      )%>%
      layout(xaxis = list(title = 'Time', range=c(min(date_range), max(date_range))), 
             yaxis = list(title = 'No. of Cases'))
  })
  
  #update category when appropriate
  observe({
    cd <- event_data("plotly_click")$x
    if (isTRUE(cd %in% regions)) category(cd)
  })
  ...
  # clear the chosen category and the selected region on back button press
  observeEvent(input$clear, category(NULL))
